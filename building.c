...
void loadBuildingData(BuildingType *building)
{
  ...
  initGhost(currGhostId++, POLTERGEIST, room, 55.65f, &ghost);
...
//Initializes a BuildingType object by initializing its ghosts and rooms members.
//input: b A pointer to the BuildingType
//output:none
void initBuilding(BuildingType *b){
  initGhostList(&b ->ghosts);
  initRoomArray(&b->rooms);
}
//Frees memory associated with a BuildingType object by cleaning up its rooms and ghosts members.
//input: b A pointer to the BuildingType 
//output: none
